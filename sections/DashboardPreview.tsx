    ...
    <section className="relative overflow-hidden bg-[#030303] px-6 py-28 text-white md:px-20">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        <p className="text-xs uppercase tracking-[0.5em] text-cyan-400">
          {isTR ? "Dashboard Önizleme" : "Dashboard Preview"}
        </p>

        <h2 className="mt-5 max-w-6xl text-4xl font-black leading-tight md:text-7xl">
          {isTR
            ? "Veri odaklı şirketler için gerçek zamanlı dashboardlar."
        ...
        >
          <div className="rounded-[2rem] border border-white/10 bg-black/70 p-6">
            <div className="flex items-center justify-between border-b border-white/10 pb-5">
              <div>
                <p className="text-sm text-zinc-500">
                  {isTR ? "2LLSOFT Konsol" : "2LLSOFT Console"}
                </p>
                <h3 className="mt-2 text-3xl font-black">
                  {isTR ? "İşletme Genel Bakış" : "Business Overview"}
                </h3>
              </div>
            ...
            <div className="mt-8 grid gap-6 md:grid-cols-4">
              {(isTR
                ? ["Gelir", "Kullanıcılar", "Projeler", "Uptime"]
                : ["Revenue", "Users", "Projects", "Uptime"]
              ).map((item, index) => (
                <div
                  key={item}
            ...
            <div className="mt-8 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
                <p className="text-zinc-500">
                  {isTR ? "Trafik Akışı" : "Traffic Flow"}
                </p>
              ...
              <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
                <p className="text-zinc-500">
                  {isTR ? "Sistem Durumu" : "System Status"}
                </p>

                <div className="mt-8 space-y-5">
                  {(isTR
                    ? ["API Aktif", "Database Aktif", "Cloud Stabil", "Güvenlik OK"]
                    : ["API Online", "Database Active", "Cloud Stable", "Security OK"]
                  ).map((item) => (
                    <div
                      key={item}
