...
export default function Projects() {
  ...
  const projects = [
    {
      name: isTR ? "Enterprise Analytics Dashboard" : "Enterprise Analytics Dashboard",
      category: isTR ? "SaaS Platformu" : "SaaS Platform",
      description: isTR
        ? "İş metrikleri, müşteri hareketleri ve operasyonel veriler için premium admin dashboard."
        : "A premium admin dashboard for tracking business metrics, client activity and operational data.",
      stack: ["Next.js", "PostgreSQL", "Prisma"],
    },
    {
      name: isTR ? "AI Otomasyon Sistemi" : "AI Automation System",
      category: isTR ? "Yapay Zeka Yazılımı" : "AI Software",
      description: isTR
        ? "Modern şirketler için AI destekli iş akışı ve otomasyon platformu."
        : "Workflow automation platform using AI-assisted processes for modern companies.",
      stack: ["OpenAI", "Node.js", "Cloud"],
    },
    {
      name: isTR ? "Mobil Ticaret Platformu" : "Mobile Commerce Platform",
      category: isTR ? "Mobil Uygulama" : "Mobile App",
      description: isTR
        ? "Temiz UI, hızlı ödeme akışı ve backend API ile ölçeklenebilir mobil ticaret deneyimi."
  ...
  return (
    <section id="projects" className="bg-[#030303] px-6 py-24 text-white md:px-20">
      ...
      <h2 className="mt-4 max-w-5xl text-4xl font-black leading-tight md:text-7xl">
        {isTR
          ? "2LLSOFT tarafından geliştirilen seçili dijital ürünler."
          : "Selected digital products engineered by 2LLSOFT."}
      </h2>

      <div className="mt-16 grid gap-8 lg:grid-cols-3">
        {projects.map((project, index) => (
          <div key={project.name} className="glass-card glass-hover overflow-hidden rounded-[2rem]">
            ...
            <div className="p-8">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {project.stack.map((item) => (
                  <span key={item} className="rounded-full border border-white/10 bg-black/30 px-4 py-2 text-sm text-zinc-300">
                    {item}
                  </span>
                ))}
              </div>
